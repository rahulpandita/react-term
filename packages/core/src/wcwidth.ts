/**
 * Unicode character width lookup (wcwidth).
 *
 * Determines display width of a Unicode codepoint:
 *   0 — zero-width (combining marks, ZWJ, control chars, variation selectors)
 *   1 — normal width (ASCII, Latin, Greek, Cyrillic, most BMP chars)
 *   2 — full-width (CJK Unified Ideographs, Hangul, fullwidth forms, etc.)
 *
 * Based on Unicode 15.1 East Asian Width + General Category properties.
 * Uses a binary search over compacted range tables for O(log n) lookup.
 */

// ---------------------------------------------------------------------------
// Zero-width codepoint ranges (combining marks, format chars, ZWJ, etc.)
// Sorted, non-overlapping. Includes:
//   - General_Category = Mn (Nonspacing_Mark)
//   - General_Category = Me (Enclosing_Mark)
//   - Soft hyphen (U+00AD)
//   - Zero-width chars (U+200B-U+200F, U+2028-U+202F, U+2060-U+2069)
//   - Variation selectors (U+FE00-U+FE0F, U+E0100-U+E01EF)
//   - Zero-width joiner/non-joiner (U+200C-U+200D)
//   - Tags (U+E0001-U+E007F)
// ---------------------------------------------------------------------------
const ZERO_WIDTH: readonly [number, number][] = [
  [0x00ad, 0x00ad], // Soft hyphen
  [0x0300, 0x036f], // Combining Diacritical Marks
  [0x0483, 0x0489], // Combining Cyrillic
  [0x0591, 0x05bd], // Hebrew combining
  [0x05bf, 0x05bf],
  [0x05c1, 0x05c2],
  [0x05c4, 0x05c5],
  [0x05c7, 0x05c7],
  [0x0600, 0x0605], // Arabic format
  [0x0610, 0x061a], // Arabic combining
  [0x061c, 0x061c],
  [0x064b, 0x065f], // Arabic combining
  [0x0670, 0x0670],
  [0x06d6, 0x06dd],
  [0x06df, 0x06e4],
  [0x06e7, 0x06e8],
  [0x06ea, 0x06ed],
  [0x070f, 0x070f],
  [0x0711, 0x0711],
  [0x0730, 0x074a], // Syriac combining
  [0x07a6, 0x07b0], // Thaana combining
  [0x07eb, 0x07f3],
  [0x07fd, 0x07fd],
  [0x0816, 0x0819],
  [0x081b, 0x0823],
  [0x0825, 0x0827],
  [0x0829, 0x082d],
  [0x0859, 0x085b],
  [0x0890, 0x0891],
  [0x0898, 0x089f],
  [0x08ca, 0x08e1],
  [0x08e3, 0x0902], // Devanagari combining (partial)
  [0x093a, 0x093a],
  [0x093c, 0x093c],
  [0x0941, 0x0948],
  [0x094d, 0x094d],
  [0x0951, 0x0957],
  [0x0962, 0x0963],
  [0x0981, 0x0981],
  [0x09bc, 0x09bc],
  [0x09c1, 0x09c4],
  [0x09cd, 0x09cd],
  [0x09e2, 0x09e3],
  [0x09fe, 0x09fe],
  [0x0a01, 0x0a02],
  [0x0a3c, 0x0a3c],
  [0x0a41, 0x0a42],
  [0x0a47, 0x0a48],
  [0x0a4b, 0x0a4d],
  [0x0a51, 0x0a51],
  [0x0a70, 0x0a71],
  [0x0a75, 0x0a75],
  [0x0a81, 0x0a82],
  [0x0abc, 0x0abc],
  [0x0ac1, 0x0ac5],
  [0x0ac7, 0x0ac8],
  [0x0acd, 0x0acd],
  [0x0ae2, 0x0ae3],
  [0x0afa, 0x0aff],
  [0x0b01, 0x0b01],
  [0x0b3c, 0x0b3c],
  [0x0b3f, 0x0b3f],
  [0x0b41, 0x0b44],
  [0x0b4d, 0x0b4d],
  [0x0b55, 0x0b56],
  [0x0b62, 0x0b63],
  [0x0b82, 0x0b82],
  [0x0bc0, 0x0bc0],
  [0x0bcd, 0x0bcd],
  [0x0c00, 0x0c00],
  [0x0c04, 0x0c04],
  [0x0c3c, 0x0c3c],
  [0x0c3e, 0x0c40],
  [0x0c46, 0x0c48],
  [0x0c4a, 0x0c4d],
  [0x0c55, 0x0c56],
  [0x0c62, 0x0c63],
  [0x0c81, 0x0c81],
  [0x0cbc, 0x0cbc],
  [0x0cbf, 0x0cbf],
  [0x0cc6, 0x0cc6],
  [0x0ccc, 0x0ccd],
  [0x0ce2, 0x0ce3],
  [0x0d00, 0x0d01],
  [0x0d3b, 0x0d3c],
  [0x0d41, 0x0d44],
  [0x0d4d, 0x0d4d],
  [0x0d62, 0x0d63],
  [0x0d81, 0x0d81],
  [0x0dca, 0x0dca],
  [0x0dd2, 0x0dd4],
  [0x0dd6, 0x0dd6],
  [0x0e31, 0x0e31],
  [0x0e34, 0x0e3a],
  [0x0e47, 0x0e4e],
  [0x0eb1, 0x0eb1],
  [0x0eb4, 0x0ebc],
  [0x0ec8, 0x0ece],
  [0x0f18, 0x0f19],
  [0x0f35, 0x0f35],
  [0x0f37, 0x0f37],
  [0x0f39, 0x0f39],
  [0x0f71, 0x0f7e],
  [0x0f80, 0x0f84],
  [0x0f86, 0x0f87],
  [0x0f8d, 0x0f97],
  [0x0f99, 0x0fbc],
  [0x0fc6, 0x0fc6],
  [0x102d, 0x1030],
  [0x1032, 0x1037],
  [0x1039, 0x103a],
  [0x103d, 0x103e],
  [0x1058, 0x1059],
  [0x105e, 0x1060],
  [0x1071, 0x1074],
  [0x1082, 0x1082],
  [0x1085, 0x1086],
  [0x108d, 0x108d],
  [0x109d, 0x109d],
  [0x1160, 0x11ff], // Hangul Jamo medial/final (zero-width in terminals)
  [0x135d, 0x135f],
  [0x1712, 0x1714],
  [0x1732, 0x1733],
  [0x1752, 0x1753],
  [0x1772, 0x1773],
  [0x17b4, 0x17b5],
  [0x17b7, 0x17bd],
  [0x17c6, 0x17c6],
  [0x17c9, 0x17d3],
  [0x17dd, 0x17dd],
  [0x180b, 0x180f], // Mongolian free variation selectors + vowel separator
  [0x1885, 0x1886],
  [0x18a9, 0x18a9],
  [0x1920, 0x1922],
  [0x1927, 0x1928],
  [0x1932, 0x1932],
  [0x1939, 0x193b],
  [0x1a17, 0x1a18],
  [0x1a1b, 0x1a1b],
  [0x1a56, 0x1a56],
  [0x1a58, 0x1a5e],
  [0x1a60, 0x1a60],
  [0x1a62, 0x1a62],
  [0x1a65, 0x1a6c],
  [0x1a73, 0x1a7c],
  [0x1a7f, 0x1a7f],
  [0x1ab0, 0x1ace],
  [0x1b00, 0x1b03],
  [0x1b34, 0x1b34],
  [0x1b36, 0x1b3a],
  [0x1b3c, 0x1b3c],
  [0x1b42, 0x1b42],
  [0x1b6b, 0x1b73],
  [0x1b80, 0x1b81],
  [0x1ba2, 0x1ba5],
  [0x1ba8, 0x1ba9],
  [0x1bab, 0x1bad],
  [0x1be6, 0x1be6],
  [0x1be8, 0x1be9],
  [0x1bed, 0x1bed],
  [0x1bef, 0x1bf1],
  [0x1c2c, 0x1c33],
  [0x1c36, 0x1c37],
  [0x1cd0, 0x1cd2],
  [0x1cd4, 0x1ce0],
  [0x1ce2, 0x1ce8],
  [0x1ced, 0x1ced],
  [0x1cf4, 0x1cf4],
  [0x1cf8, 0x1cf9],
  [0x1dc0, 0x1dff], // Combining Diacritical Marks Supplement
  [0x200b, 0x200f], // Zero-width space, ZWNJ, ZWJ, LRM, RLM
  [0x2028, 0x202e], // Line/paragraph separator, bidi controls
  [0x2060, 0x2069], // Word joiner, invisible operators, bidi isolates
  [0x206a, 0x206f], // Deprecated formatting
  [0x20d0, 0x20f0], // Combining marks for symbols
  [0x2cef, 0x2cf1],
  [0x2d7f, 0x2d7f],
  [0x2de0, 0x2dff],
  [0xa66f, 0xa672],
  [0xa674, 0xa67d],
  [0xa69e, 0xa69f],
  [0xa6f0, 0xa6f1],
  [0xa802, 0xa802],
  [0xa806, 0xa806],
  [0xa80b, 0xa80b],
  [0xa825, 0xa826],
  [0xa82c, 0xa82c],
  [0xa8c4, 0xa8c5],
  [0xa8e0, 0xa8f1],
  [0xa8ff, 0xa8ff],
  [0xa926, 0xa92d],
  [0xa947, 0xa951],
  [0xa980, 0xa982],
  [0xa9b3, 0xa9b3],
  [0xa9b6, 0xa9b9],
  [0xa9bc, 0xa9bd],
  [0xa9e5, 0xa9e5],
  [0xaa29, 0xaa2e],
  [0xaa31, 0xaa32],
  [0xaa35, 0xaa36],
  [0xaa43, 0xaa43],
  [0xaa4c, 0xaa4c],
  [0xaa7c, 0xaa7c],
  [0xaab0, 0xaab0],
  [0xaab2, 0xaab4],
  [0xaab7, 0xaab8],
  [0xaabe, 0xaabf],
  [0xaac1, 0xaac1],
  [0xaaec, 0xaaed],
  [0xaaf6, 0xaaf6],
  [0xabe5, 0xabe5],
  [0xabe8, 0xabe8],
  [0xabed, 0xabed],
  [0xfb1e, 0xfb1e],
  [0xfe00, 0xfe0f], // Variation Selectors
  [0xfe20, 0xfe2f], // Combining Half Marks
  [0xfeff, 0xfeff], // BOM / ZWNBSP
  [0xfff9, 0xfffb], // Interlinear annotation
  [0x101fd, 0x101fd],
  [0x102e0, 0x102e0],
  [0x10376, 0x1037a],
  [0x10a01, 0x10a03],
  [0x10a05, 0x10a06],
  [0x10a0c, 0x10a0f],
  [0x10a38, 0x10a3a],
  [0x10a3f, 0x10a3f],
  [0x10ae5, 0x10ae6],
  [0x10d24, 0x10d27],
  [0x10eab, 0x10eac],
  [0x10efd, 0x10eff],
  [0x10f46, 0x10f50],
  [0x10f82, 0x10f85],
  [0x11001, 0x11001],
  [0x11038, 0x11046],
  [0x11070, 0x11070],
  [0x11073, 0x11074],
  [0x1107f, 0x11081],
  [0x110b3, 0x110b6],
  [0x110b9, 0x110ba],
  [0x110c2, 0x110c2],
  [0x11100, 0x11102],
  [0x11127, 0x1112b],
  [0x1112d, 0x11134],
  [0x11173, 0x11173],
  [0x11180, 0x11181],
  [0x111b6, 0x111be],
  [0x111c9, 0x111cc],
  [0x111cf, 0x111cf],
  [0x1122f, 0x11231],
  [0x11234, 0x11234],
  [0x11236, 0x11237],
  [0x1123e, 0x1123e],
  [0x11241, 0x11241],
  [0x112df, 0x112df],
  [0x112e3, 0x112ea],
  [0x11300, 0x11301],
  [0x1133b, 0x1133c],
  [0x11340, 0x11340],
  [0x11366, 0x1136c],
  [0x11370, 0x11374],
  [0x11438, 0x1143f],
  [0x11442, 0x11444],
  [0x11446, 0x11446],
  [0x1145e, 0x1145e],
  [0x114b3, 0x114b8],
  [0x114ba, 0x114ba],
  [0x114bf, 0x114c0],
  [0x114c2, 0x114c3],
  [0x115b2, 0x115b5],
  [0x115bc, 0x115bd],
  [0x115bf, 0x115c0],
  [0x115dc, 0x115dd],
  [0x11633, 0x1163a],
  [0x1163d, 0x1163d],
  [0x1163f, 0x11640],
  [0x116ab, 0x116ab],
  [0x116ad, 0x116ad],
  [0x116b0, 0x116b5],
  [0x116b7, 0x116b7],
  [0x1171d, 0x1171f],
  [0x11722, 0x11725],
  [0x11727, 0x1172b],
  [0x1182f, 0x11837],
  [0x11839, 0x1183a],
  [0x1193b, 0x1193c],
  [0x1193e, 0x1193e],
  [0x11943, 0x11943],
  [0x119d4, 0x119d7],
  [0x119da, 0x119db],
  [0x119e0, 0x119e0],
  [0x11a01, 0x11a0a],
  [0x11a33, 0x11a38],
  [0x11a3b, 0x11a3e],
  [0x11a47, 0x11a47],
  [0x11a51, 0x11a56],
  [0x11a59, 0x11a5b],
  [0x11a8a, 0x11a96],
  [0x11a98, 0x11a99],
  [0x11c30, 0x11c36],
  [0x11c38, 0x11c3d],
  [0x11c3f, 0x11c3f],
  [0x11c92, 0x11ca7],
  [0x11caa, 0x11cb0],
  [0x11cb2, 0x11cb3],
  [0x11cb5, 0x11cb6],
  [0x11d31, 0x11d36],
  [0x11d3a, 0x11d3a],
  [0x11d3c, 0x11d3d],
  [0x11d3f, 0x11d45],
  [0x11d47, 0x11d47],
  [0x11d90, 0x11d91],
  [0x11d95, 0x11d95],
  [0x11d97, 0x11d97],
  [0x11ef3, 0x11ef4],
  [0x11f00, 0x11f01],
  [0x11f36, 0x11f3a],
  [0x11f40, 0x11f40],
  [0x11f42, 0x11f42],
  [0x13430, 0x1345f],
  [0x13440, 0x13440],
  [0x13447, 0x13455],
  [0x16af0, 0x16af4],
  [0x16b30, 0x16b36],
  [0x16f4f, 0x16f4f],
  [0x16f8f, 0x16f92],
  [0x16fe4, 0x16fe4],
  [0x1bc9d, 0x1bc9e],
  [0x1bca0, 0x1bca3],
  [0x1cf00, 0x1cf2d],
  [0x1cf30, 0x1cf46],
  [0x1d167, 0x1d169],
  [0x1d173, 0x1d182],
  [0x1d185, 0x1d18b],
  [0x1d1aa, 0x1d1ad],
  [0x1d242, 0x1d244],
  [0x1da00, 0x1da36],
  [0x1da3b, 0x1da6c],
  [0x1da75, 0x1da75],
  [0x1da84, 0x1da84],
  [0x1da9b, 0x1da9f],
  [0x1daa1, 0x1daaf],
  [0x1e000, 0x1e006],
  [0x1e008, 0x1e018],
  [0x1e01b, 0x1e021],
  [0x1e023, 0x1e024],
  [0x1e026, 0x1e02a],
  [0x1e08f, 0x1e08f],
  [0x1e130, 0x1e136],
  [0x1e2ae, 0x1e2ae],
  [0x1e2ec, 0x1e2ef],
  [0x1e4ec, 0x1e4ef],
  [0x1e8d0, 0x1e8d6],
  [0x1e944, 0x1e94a],
  [0xe0001, 0xe0001], // Language tag
  [0xe0020, 0xe007f], // Tag components
  [0xe0100, 0xe01ef], // Variation Selectors Supplement
];

// ---------------------------------------------------------------------------
// Full-width / wide codepoint ranges (East_Asian_Width = W or F)
// Sorted, non-overlapping.
// ---------------------------------------------------------------------------
const WIDE: readonly [number, number][] = [
  [0x1100, 0x115f], // Hangul Jamo initial consonants
  [0x231a, 0x231b], // Watch, Hourglass
  [0x2329, 0x232a], // Angle brackets
  [0x23e9, 0x23ec], // Fast-forward/rewind
  [0x23f0, 0x23f0], // Alarm clock
  [0x23f3, 0x23f3], // Hourglass flowing
  [0x25fd, 0x25fe], // Medium small squares
  [0x2614, 0x2615], // Umbrella, hot beverage
  [0x2648, 0x2653], // Zodiac signs
  [0x267f, 0x267f], // Wheelchair
  [0x2693, 0x2693], // Anchor
  [0x26a1, 0x26a1], // High voltage
  [0x26aa, 0x26ab], // Medium circles
  [0x26bd, 0x26be], // Soccer, baseball
  [0x26c4, 0x26c5], // Snowman, sun behind cloud
  [0x26ce, 0x26ce], // Ophiuchus
  [0x26d4, 0x26d4], // No entry
  [0x26ea, 0x26ea], // Church
  [0x26f2, 0x26f3], // Fountain, golf
  [0x26f5, 0x26f5], // Sailboat
  [0x26fa, 0x26fa], // Tent
  [0x26fd, 0x26fd], // Fuel pump
  [0x2702, 0x2702], // Scissors
  [0x2705, 0x2705], // Check mark
  [0x2708, 0x270d], // Airplane through writing hand
  [0x270f, 0x270f], // Pencil
  [0x2712, 0x2712], // Black nib
  [0x2714, 0x2714], // Check mark
  [0x2716, 0x2716], // X mark
  [0x271d, 0x271d], // Latin cross
  [0x2721, 0x2721], // Star of David
  [0x2728, 0x2728], // Sparkles
  [0x2733, 0x2734], // Eight-spoked asterisk
  [0x2744, 0x2744], // Snowflake
  [0x2747, 0x2747], // Sparkle
  [0x274c, 0x274c], // Cross mark
  [0x274e, 0x274e], // Cross mark
  [0x2753, 0x2755], // Question marks
  [0x2757, 0x2757], // Exclamation mark
  [0x2763, 0x2764], // Heart exclamation, heavy heart
  [0x2795, 0x2797], // Plus, minus, divide
  [0x27a1, 0x27a1], // Right arrow
  [0x27b0, 0x27b0], // Curly loop
  [0x27bf, 0x27bf], // Double curly loop
  [0x2934, 0x2935], // Arrow up-right, down-right
  [0x2b05, 0x2b07], // Left/up/down arrows
  [0x2b1b, 0x2b1c], // Black/white large squares
  [0x2b50, 0x2b50], // Star
  [0x2b55, 0x2b55], // Heavy large circle
  [0x2e80, 0x2e99], // CJK Radicals Supplement
  [0x2e9b, 0x2ef3],
  [0x2f00, 0x2fd5], // Kangxi Radicals
  [0x2ff0, 0x303e], // Ideographic Description + CJK Symbols
  [0x3041, 0x3096], // Hiragana
  [0x3099, 0x30ff], // Hiragana/Katakana
  [0x3105, 0x312f], // Bopomofo
  [0x3131, 0x318e], // Hangul Compatibility Jamo
  [0x3190, 0x31e3], // Kanbun + CJK Strokes
  [0x31ef, 0x321e], // CJK Enclosed
  [0x3220, 0x3247], // Enclosed CJK
  [0x3250, 0x4dbf], // CJK Unified Ext A + more
  [0x4e00, 0xa48c], // CJK Unified Ideographs
  [0xa490, 0xa4c6], // Yi Radicals
  [0xa960, 0xa97c], // Hangul Jamo Extended-A
  [0xac00, 0xd7a3], // Hangul Syllables
  [0xf900, 0xfaff], // CJK Compatibility Ideographs
  [0xfe10, 0xfe19], // Vertical Forms
  [0xfe30, 0xfe6b], // CJK Compatibility Forms + Small Forms
  [0xff01, 0xff60], // Fullwidth ASCII variants
  [0xffe0, 0xffe6], // Fullwidth signs
  [0x16fe0, 0x16fe3], // Ideographic Symbols
  [0x16ff0, 0x16ff1],
  [0x17000, 0x187f7], // Tangut
  [0x18800, 0x18cd5], // Tangut Components
  [0x18d00, 0x18d07],
  [0x1aff0, 0x1aff3],
  [0x1aff5, 0x1affb],
  [0x1affd, 0x1affe],
  [0x1b000, 0x1b122], // Kana Supplement + Extended
  [0x1b132, 0x1b132],
  [0x1b150, 0x1b152],
  [0x1b155, 0x1b155],
  [0x1b164, 0x1b167],
  [0x1b170, 0x1b2fb], // Nüshu
  [0x1f004, 0x1f004], // Mahjong tile
  [0x1f0cf, 0x1f0cf], // Playing card
  [0x1f18e, 0x1f18e], // AB button
  [0x1f191, 0x1f19a], // Squared symbols
  [0x1f200, 0x1f202], // Enclosed ideographic
  [0x1f210, 0x1f23b],
  [0x1f240, 0x1f248],
  [0x1f250, 0x1f251],
  [0x1f260, 0x1f265],
  [0x1f300, 0x1f320], // Misc Symbols and Pictographs
  [0x1f32d, 0x1f335],
  [0x1f337, 0x1f37c],
  [0x1f37e, 0x1f393],
  [0x1f3a0, 0x1f3ca],
  [0x1f3cf, 0x1f3d3],
  [0x1f3e0, 0x1f3f0],
  [0x1f3f4, 0x1f3f4],
  [0x1f3f8, 0x1f43e],
  [0x1f440, 0x1f440],
  [0x1f442, 0x1f4fc],
  [0x1f4ff, 0x1f53d],
  [0x1f54b, 0x1f54e],
  [0x1f550, 0x1f567],
  [0x1f57a, 0x1f57a],
  [0x1f595, 0x1f596],
  [0x1f5a4, 0x1f5a4],
  [0x1f5fb, 0x1f64f], // Transport/Map + Emoticons
  [0x1f680, 0x1f6c5], // Transport
  [0x1f6cc, 0x1f6cc],
  [0x1f6d0, 0x1f6d2],
  [0x1f6d5, 0x1f6d7],
  [0x1f6dc, 0x1f6df],
  [0x1f6eb, 0x1f6ec],
  [0x1f6f4, 0x1f6fc],
  [0x1f7e0, 0x1f7eb],
  [0x1f7f0, 0x1f7f0],
  [0x1f90c, 0x1f93a],
  [0x1f93c, 0x1f945],
  [0x1f947, 0x1f9ff],
  [0x1fa00, 0x1fa53],
  [0x1fa60, 0x1fa6d],
  [0x1fa70, 0x1fa7c],
  [0x1fa80, 0x1fa89],
  [0x1fa8f, 0x1fac5],
  [0x1face, 0x1fadb],
  [0x1fae0, 0x1fae8],
  [0x1faf0, 0x1faf8],
  [0x20000, 0x2fffd], // CJK Unified Ext B-F
  [0x30000, 0x3fffd], // CJK Unified Ext G-I
];

/**
 * Binary search: returns true if `cp` falls within any range in `table`.
 */
function inRanges(table: readonly [number, number][], cp: number): boolean {
  let lo = 0;
  let hi = table.length - 1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    const [start, end] = table[mid];
    if (cp < start) {
      hi = mid - 1;
    } else if (cp > end) {
      lo = mid + 1;
    } else {
      return true;
    }
  }
  return false;
}

// ---------------------------------------------------------------------------
// BMP flat lookup table (U+0000 – U+FFFF): O(1) per codepoint.
// 64 KB Uint8Array, each byte stores the width (0, 1, or 2).
// Built once at module load from the range tables above.
// Supplementary planes (U+10000+) still use binary search.
// ---------------------------------------------------------------------------
const BMP_TABLE = new Uint8Array(0x10000);

// Initialize: default width is 1 for printable, 0 for controls
function initBmpTable(): void {
  // Default: all printable = 1
  BMP_TABLE.fill(1);

  // C0 controls
  for (let cp = 0; cp < 0x20; cp++) BMP_TABLE[cp] = 0;
  // DEL + C1 controls
  for (let cp = 0x7f; cp < 0xa0; cp++) BMP_TABLE[cp] = 0;

  // Mark zero-width ranges
  for (const [start, end] of ZERO_WIDTH) {
    if (start > 0xffff) break; // rest are supplementary
    const clampedEnd = Math.min(end, 0xffff);
    for (let cp = start; cp <= clampedEnd; cp++) BMP_TABLE[cp] = 0;
  }

  // Mark wide ranges
  for (const [start, end] of WIDE) {
    if (start > 0xffff) break; // rest are supplementary
    const clampedEnd = Math.min(end, 0xffff);
    for (let cp = start; cp <= clampedEnd; cp++) BMP_TABLE[cp] = 2;
  }
}

initBmpTable();

// Supplementary-only range tables (U+10000+) for binary search
const ZERO_WIDTH_SUPP = ZERO_WIDTH.filter(([start]) => start >= 0x10000);
const WIDE_SUPP = WIDE.filter(([start]) => start >= 0x10000);

/**
 * Return the display width of a Unicode codepoint.
 *
 * @returns 0 for zero-width, 1 for normal, 2 for full-width/wide
 */
export function wcwidth(cp: number): number {
  // BMP: O(1) table lookup
  if (cp < 0x10000) return BMP_TABLE[cp];

  // Supplementary planes: binary search
  if (inRanges(ZERO_WIDTH_SUPP, cp)) return 0;
  if (inRanges(WIDE_SUPP, cp)) return 2;
  return 1;
}

/**
 * Returns true if the codepoint is zero-width and should not advance
 * the cursor (combining marks, variation selectors, ZWJ, etc.).
 * Excludes C0/C1 controls and soft hyphen (< U+0300).
 */
export function isCombining(cp: number): boolean {
  return cp >= 0x0300 && wcwidth(cp) === 0;
}

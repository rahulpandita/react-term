...
import type { TerminalHandle, TerminalPaneHandle, PaneLayout } from '@react-term/react';
...
function showColors(write: WriteFunc) {
  ...
  write('  \x1b[1mBold\x1b[0m  \x1b[2mDim\x1b[0m  \x1b[3mItalic\x1b[0m  \x1b[4mUnderline\x1b[0m  \x1b[7mInverse\x1b[0m  \x1b[9mStrikethrough\x1b[0m\r\n');
  write('\r\n');
}
...
function runBenchmark(write: WriteFunc) {
  ...
  for (let i = 0; i < 10000; i++) {
    ...
    lines.push(`\x1b[${color}m[${String(i).padStart(5, '0')}] The quick brown fox jumps over the lazy dog | ABCDEFghijklmnop 1234567890\x1b[0m`);
  }
  write(lines.join('\r\n'));
...
function App() {
  ...
  const themeCallbackRef = useRef<((dark: boolean) => void) | null>(null);
  const fps = useFps();
  ...
  const connectWs = useCallback(() => {
    ...
    ws.onclose = () => {
      ...
      if (modeRef.current === 'pty') {
        ...
        termRef.current?.write('\x1b[90mFalling back to local echo mode. Reconnecting in 3s...\x1b[0m\r\n');

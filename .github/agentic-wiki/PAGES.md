...
*{ Document the @next_term/core package (packages/core/). Describe CellGrid (the SharedArrayBuffer-backed cell grid, cell packing format with 4×Uint32 per cell / 16 bytes including foreground and background RGB words, dirty rows tracking, cursor data), Buffer and BufferSet (scroll regions, tab stops, cursor save/restore, scrollback, alternate screen), VTParser (the VT100/ANSI state machine, including OSC sequence callbacks: clipboard access via OSC 52, indexed-colour palette set/query via OSC 4/104, working-directory reporting via OSC 7, and dynamic color query/set for foreground/background/cursor colors via OSC 10/11/12), and the exported types (CursorState, TerminalOptions, Theme, DirtyState). Derive method and free-function signatures directly from source: CellGrid.setCell includes foreground/background indexes and optional RGB arguments, while extractText is a standalone range-selection function rather than a CellGrid method. Include a Mermaid diagram showing the relationships between CellGrid, Buffer, BufferSet, and VTParser. Link to the source files at packages/core/src/. }*

## Web Package

*{ Document the @next_term/web package (packages/web/). Cover: WebTerminal as the main orchestrator, WorkerBridge (parser worker lifecycle, flow control with high/low watermarks, SAB vs Transferable fallback), RenderBridge (OffscreenCanvas render worker, feature detection), Canvas2DRenderer, WebGLRenderer (instanced rendering, glyph atlas, two primary grid passes plus separate cursor/selection/highlight overlays), InputHandler, AccessibilityManager, SharedWebGLContext (shared context for multi-pane, bypasses Chrome 16-context limit), and the addons system (SearchAddon, WebLinksAddon, FitAddon). Derive every callback and addon method signature directly from source, including onResize receiving one { cols, rows } object, SearchAddon.clearSearch(), and WebLinksAddon receiving a single URL string. Do not invent performance figures; include quantitative results only when backed by a repository benchmark or test. Include a Mermaid sequence diagram showing the write data flow from WebTerminal through WorkerBridge to the parser worker and back. Link to packages/web/src/. }*
...
*{ Document the @next_term/react package (packages/react/). Derive every prop type and default directly from packages/react/src/Terminal.tsx: onData receives Uint8Array, onResize receives one { cols, rows } object, fontSize defaults to 16, fontFamily defaults to "'Courier New', monospace", scrollInputMode is "terminal" | "page", renderMode is "auto" | "offscreen" | "main", and renderer is "auto" | "webgl" | "canvas2d" rather than a custom renderer instance. Describe TerminalHandle (write, resize, focus, blur, fit). Derive PaneLayout directly from packages/react/src/pane-layout.ts using the discriminated type field ("single", "horizontal", "vertical"); sizes are fractional weights such as [0.5, 0.5]. Describe TerminalPaneHandle (getTerminal, getPaneIds) and include type-correct React examples with literal generic angle brackets, JSX tags, and arrow functions preserved. Link to packages/react/src/. }*

## Native Package

*{ Document the @next_term/native package (packages/native/). Derive NativeTerminalProps directly from packages/native/src/NativeTerminal.tsx: onData receives Uint8Array, onResize receives one { cols, rows } object, and style uses the source-declared Record<string, unknown> type rather than ViewStyle. Cover TerminalSurface, SkiaRenderer, and the TurboModule interface. Derive GestureState and GestureConfig exactly from packages/core/src/gesture-handler.ts; GestureConfig contains callbacks rather than speed/threshold settings. Derive KeyboardHandler directly from packages/native/src/input/KeyboardHandler.ts: its constructor requires an onData callback, handleKeyPress returns void, and keyToSequence returns the sequence string. Include type-correct examples and a component-layer diagram. }*

# Getting Started

*{ Write a complete getting-started guide for react-term. Cover: prerequisites (Node.js, pnpm), installation of @next_term/react and @next_term/web into an existing React project, the minimal Terminal component usage example, configuring cross-origin isolation headers required for SharedArrayBuffer (COOP/COEP), the three rendering strategies and when each is selected automatically, and how to connect to a PTY/WebSocket by handling the onData callback. Derive callback signatures directly from source, including onResize receiving one { cols, rows } object. Include code snippets for installation, configuration, and usage. Preserve literal shell and TypeScript/JSX syntax: do not wrap package names in backticks inside code fences or replace angle brackets and arrow tokens. }*
...
*{ Document the rendering subsystem of react-term in depth. Cover: the rendering strategy auto-detection logic (SAB + OffscreenCanvas → Full Worker; SAB only → Parser Worker + main thread WebGL2; fallback → Canvas 2D on main thread), WebGLRenderer details (instanced rendering, alpha-only glyph atlas, draw call budget), Canvas2DRenderer as the universal fallback, the OffscreenCanvas render worker via RenderBridge, SharedWebGLContext for multi-terminal sharing. Derive instance-buffer field layouts directly from shader attributes and buffer writes; cell width and height are uniforms, not per-instance fields. Include a Mermaid flowchart showing the strategy decision tree. Reference packages/web/src/renderer.ts, packages/web/src/webgl-renderer.ts, and packages/web/src/render-bridge.ts. }*

# Accessibility

*{ Document the accessibility support in react-term. Cover: AccessibilityManager (packages/web/src/accessibility.ts) which maintains a parallel DOM with ARIA attributes, extractRowText utility, screen reader support, how the accessibility tree is kept in sync with terminal output. Include any relevant ARIA roles and attributes used. Preserve literal opening and closing HTML tags inside code fences. }*

# Contributing

*{ Write a contributing guide for react-term. Cover: cloning the monorepo, installing dependencies with pnpm, the package workspace structure (packages/core, packages/web, packages/react, packages/native, packages/demo), running tests (pnpm test / vitest), running the demo (pnpm dev), adding new tests, code style expectations. Describe the test layout per package and the vitest configuration. Use only valid, fully qualified links and do not emit placeholders or redacted URLs. }*
